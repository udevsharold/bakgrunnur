...
-(void)assertionManager:(id)manager willExpireAssertionsSoonForProcess:(RBProcess *)proc expirationTime:(double)time{
    if (enabled){
       ...
       // }
        ...
        if ([enabledIdentifier containsObject:proc.identity.embeddedApplicationIdentifier] && ![bakgrunnurRetiring[proc.identity.embeddedApplicationIdentifier] boolValue ]){
            ...
            HBLogDebug(@"Wont Expire");
            return;
        }else if ([bakgrunnurRetiring[proc.identity.embeddedApplicationIdentifier] boolValue] && ![procManager isActiveProcess:proc]){
            [bakgrunnurRetiring removeObjectForKey:proc.identity.embeddedApplicationIdentifier];
            HBLogDebug(@"Removed: %@", bakgrunnurRetiring);
        }
    }
    %orig;
}
...
+(RBSProcessState *)_clientStateForServerState:(RBProcessState *)state process:(RBProcess *)proc{
   ...
   // MSHookIvar<unsigned char>(state, "_terminationResistance") = 3;
    if (enabled){
        ...
        if ([enabledIdentifier containsObject:state.identity.embeddedApplicationIdentifier] && ![bakgrunnurRetiring[proc.identity.embeddedApplicationIdentifier] boolValue]){
